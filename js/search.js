            ...
            $("#search-box").selectize({
                ...
                render: {
                    option: function (item, escape) {
                        return `
                        <div>
                        <p>${escape(item.name)} (${escape(item.land)}, ${escape(item.region)}, ${escape(item.weingut)}) <span class="badge badge-custom-${item.type}">${escape(item.type)}</span> <span class="badge badge-secondary">${escape(item.jahr)}</span></p>
</div>
                        `;
                    }
                },
